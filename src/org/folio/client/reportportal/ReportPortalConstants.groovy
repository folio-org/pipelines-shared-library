...
class ReportPortalConstants {
  ...
  public static final String CYPRESS_EXEC_PARAM_TEMPLATE = '''--reporter "@reportportal/agent-js-cypress" \
   --reporter-options \
   endpoint="${api_url}",apiKey="${api_key}",project="${project_name}",description="${description}",launch="${launch_name}",launchId="${launch_id}",mode="DEFAULT",isLaunchMergeRequired="true",debug="true",attributes="${attributes}"
'''
}
