...
class Deployment extends GeneralParameters {
    ...
    Deployment(Object steps, String okapiUrl, String stripesUrl, String repository, String branch, OkapiTenant tenant, OkapiUser admin_user, Email email, String kb_api_key, reindex_elastic_search, recreate_index_elastic_search, saved_to_s3_install_json, saved_to_s3_okapi_install_json) {
        ...
        this.saved_to_s3_okapi_install_json = saved_to_s3_okapi_install_json
    ...
    void restoreFromBackup() {
        if (tenant) {
            ...
            discoveryList = installCustomJsonsUtility.customBuildDiscoveryList(saved_to_s3_okapi_install_json)
            okapi.cleanupServicesRegistration()
            okapi.registerServices(discoveryList)
            okapi.publishModuleDescriptors(enableList)
            okapi.enableDisableUpgradeModulesForTenant(tenant, okapi.buildInstallList(["okapi"], "enable"))
