                            ...
                            catchError (buildResult: 'FAILURE', stageResult: 'FAILURE') {
                                if (params.testrailRunID && params.testrailProjectID) {
                                    ...
                                    withCredentials([usernamePassword(credentialsId: 'testrail-ut56', passwordVariable: 'TESTRAIL_PASSWORD', usernameVariable: 'TESTRAIL_USERNAME')]) {
                                        sh "yarn global add cy2@latest && env && pwd && ls -a \$HOME && /usr/local/bin/cy2 run --config projectId=stripes --parallel --record --key somekey --ci-build-id ${BUILD_NUMBER} --browser ${browserName} ${params.cypressParameters}"
                                    }
                                } else {
                                    sh "yarn global add cy2@latest && env && pwd && ls -a \$HOME && /usr/local/bin/cy2 run --config projectId=stripes --parallel --record --key somekey --ci-build-id ${BUILD_NUMBER} --browser ${browserName} ${params.cypressParameters}"
                                }
