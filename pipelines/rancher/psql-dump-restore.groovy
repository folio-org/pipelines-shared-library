#!groovy
@Library('pipelines-shared-library@RANCHER-396') _
...
String db_backup_name = params.restore_postgresql_from_backup ? params.restore_postgresql_backup_name : "backup_${date_time}-${started_by_user}"
OkapiUser admin_user = okapiSettings.adminUser()
String s3_postgres_backups_bucket_name = "s3://folio-postgresql-backups"
                ...
                docker.image(Constants.PSQL_DUMP_DOCKER_CLIENT).inside("-u 0:0 --entrypoint=") {
                    ...
                    try {
                        if (params.restore_postgresql_from_backup == false) {
                            psqlDumpMethods.backupHelmInstall(env.BUILD_ID, Constants.FOLIO_HELM_REPOSITORY_NAME, Constants.PSQL_DUMP_HELM_CHART_NAME, Constants.PSQL_DUMP_HELM_INSTALL_CHART_VERSION, params.rancher_project_name, params.rancher_cluster_name, db_backup_name, params.tenant_id_to_backup_modules_versions, admin_user.username, admin_user.password)
                            psqlDumpMethods.savePlatformCompleteImageTag(params.rancher_project_name, params.rancher_cluster_name, db_backup_name, s3_postgres_backups_bucket_name, params.tenant_id_to_backup_modules_versions)
