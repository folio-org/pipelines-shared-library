...
properties([
  ...
  parameters([
    string(name: 'baseUrl', description: 'Choose what you want', defaultValue:"https://folio-snapshot.dev.folio.org"),
    string(name: 'OKAPI_HOST', description: 'Choose what you want', defaultValue:"https://folio-snapshot-okapi.dev.folio.org"),
...
node {
  stage('Start') {
    ...
    stage('Get uid, gid'){
      def currentUID = sh returnStdout: true, script: 'id -u'
      def currentGID = sh returnStdout: true, script: 'id -g'
    }
    try {
      docker.image('cypress/browsers:latest').inside('-u 0:0 --entrypoint=') {
        ...
        stage('generate allure-report') {
          dir('stripes-testing'){
            def allure_home = tool name: '2.17.2', type: 'allure'
