      ...
      spec {
        ...
        container {
          ...
          command = [
            ...
            "-c",
            <<-EOF
              curl -X PUT "http://opensearch-${var.rancher_project_name}:9200/_cluster/settings" -H "Content-Type: application/json" -d '{"persistent": {"action.auto_create_index": "false"}}' && curl -X PUT http://opensearch-${var.rancher_project_name}/_cluster/settings -H "Content-Type: application/json" -d '{ "persistent": { "cluster.max_shards_per_node": "1500" } }'
            EOF
          ]
        }
      }
