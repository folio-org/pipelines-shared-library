resource "rancher2_secret" "keycloak-credentials" {
  data = {
    KC_DB_URL_HOST                  = base64encode(var.pg_embedded ? (var.rancher_project_name == "cikarate" ? "postgresql-cikarate-tests" : local.pg_service_writer) : module.rds[0].cluster_endpoint)
