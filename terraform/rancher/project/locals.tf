...
EOF
  ...
  keycloak_custom_snapshot   = can(regex("^.+-SNAPSHOT\\.[0-9a-f]{7}$", var.keycloak_version)) || can(regex("^.+-SNAPSHOT\\.pr\\d+\\.[0-9a-f]{7}$", var.keycloak_version))
  ...
  kong_custom_snapshot       = can(regex("^.+-SNAPSHOT\\.[0-9a-f]{7}$", var.kong_version)) || can(regex("^.+-SNAPSHOT\\.pr\\d+\\.[0-9a-f]{7}$", var.kong_version))
