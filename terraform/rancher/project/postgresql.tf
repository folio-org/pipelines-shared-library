...
resource "rancher2_app" "pgadmin4" {
  ...
  answers = {
    ...
    "serverDefinitions.servers.pg"                                      = jsonencode(tomap("name","pg_folio","host","postgresql","MaintenanceDB", var.pg_dbname,"port", "5432", "password", var.pg_password, "username", var.pg_username))
