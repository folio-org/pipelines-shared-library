resource "rancher2_secret" "kong-credentials" {
  data = {
    ...
    KONG_PG_HOST     = base64encode(var.pg_embedded ? (var.rancher_project_name == "cikarate" ? "postgresql-cikarate-tests" : local.pg_service_writer) : module.rds[0].cluster_endpoint)
...
resource "helm_release" "kong" {
  count      = var.eureka ? 1 : 0
  chart      = "kong"
  depends_on = [rancher2_secret.db-credentials, rancher2_secret.db-credentials-cikarate, helm_release.postgresql, helm_release.postgresql_cikarate, rancher2_secret.kong-credentials, module.rds.cluster_instances, postgresql_database.eureka_kong]
...
postgresql:
  enabled: false
  external:
    host: ${var.pg_embedded ? (var.rancher_project_name == "cikarate" ? "postgresql-cikarate-tests" : local.pg_service_writer) : module.rds[0].cluster_endpoint}
    port: 5432
    user: kong
    password: ""
    database: kong
   ...
   - host: ${join(".", [join("-", ["ecs", data.rancher2_cluster.this.name, var.rancher_project_name, "kong"]), var.root_domain])}
     http:
       ...
       - backend:
         ...
         path: /*
         pathType: ImplementationSpecific
   - host: ${join(".", [join("-", ["ecs2", data.rancher2_cluster.this.name, var.rancher_project_name, "kong"]), var.root_domain])}
     http:
       paths:
       - backend:
           service:
