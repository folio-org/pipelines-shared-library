resource "rancher2_secret" "kong-credentials" {
  data = {
    ...
    KONG_PG_HOST     = base64encode(var.pg_embedded ? (contains(["cikarate", "cicypress", "cypress", "karate"], var.rancher_project_name) ? "postgresql-${var.rancher_project_name}-eureka" : local.pg_service_writer) : module.rds[0].cluster_endpoint)
...
postgresql:
  enabled: false
  external:
    host: ${var.pg_embedded ? (contains(["cikarate", "cicypress", "cypress", "karate"], var.rancher_project_name) ? "postgresql-${var.rancher_project_name}-eureka" : local.pg_service_writer) : module.rds[0].cluster_endpoint}
    port: 5432
    user: kong
    password: ""
    database: kong
...
kong:
  ...
  - name: KONG_SEND_TIMEOUT
    value: "600000"
  - name: KONG_RECEIVE_TIMEOUT
    value: "600000"
  - name: KONG_UPSTREAM_ALGORITHM
    value: "round-robin"
  - name: KONG_NGINX_UPSTREAM_IP_HASH
    value: "off"
