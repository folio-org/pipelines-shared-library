resource "rancher2_secret" "db-connect-modules" {
  ...
  data = {
    ...
    DB_HOST                = base64encode(var.pg_embedded ? "postgresql-${var.rancher_project_name}" : module.rds[0].this_rds_cluster_endpoint)
    ...
    DB_DATABASE            = base64encode(var.pg_embedded ? var.pg_dbname : module.rds_database_name_cli[0].result)
    ...
    ELASTICSEARCH_URL      = base64encode(var.es_embedded ? "http://opensearch-${var.rancher_project_name}:9200" : "https://${module.aws_es[0].endpoint}:443")
    ELASTICSEARCH_HOST     = base64encode(var.es_embedded ? "" : module.aws_es[0].endpoint)
