...
resource "rancher2_secret" "eureka_common" {
  ...
  data = {
    ...
    KC_ADMIN_USERNAME               = base64encode(lookup(data.rancher2_secret.keycloak_credentials[0].data, "KEYCLOAK_ADMIN_USER", "admin"))
    KC_ADMIN_PASSWORD               = base64encode(lookup(data.rancher2_secret.keycloak_credentials[0].data, "KEYCLOAK_ADMIN_PASSWORD", ""))
