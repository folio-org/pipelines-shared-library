...
// Public API methods
void buildAndDeploy(RancherNamespace namespace, EurekaTenant tenant, List<EurekaModule> addUIComponents,
                    List<EurekaModule> removeUIComponents, boolean enableEcsRequests = false) {
  build(tenant, addUIComponents, removeUIComponents, enableEcsRequests)
  deploy(namespace, tenant)
}

void build(EurekaTenant tenant, List<EurekaModule> addUIComponents, List<EurekaModule> removeUIComponents, boolean enableEcsRequests) {
  ...
  podTemplates.stripesAgent {
    ...
    _preBuildConfigure(tenant, addUIComponents, removeUIComponents, enableEcsRequests)
...
private void _preBuildConfigure(EurekaTenant tenant, List<EurekaModule> addUIComponents,
                                List<EurekaModule> removeUIComponents, boolean enableEcsRequests) {
  stage('[UI] Pre-build configuration') {
    ...
    if (removeUIComponents) {
      _removeUIComponents(packageJson, removeUIComponents)
    }

    if (addUIComponents) {
      _addUIComponents(packageJson, addUIComponents)
    }
...
private void _renderConfig(String stripesConfig, EurekaTenant tenant, TenantUi tenantUi, boolean enableEcsRequests) {
  stage('Render Config') {
    ...
    Map tplData = [
      ...
      aboutInstallMsg  : String.format("'%s'", "Branch: ${tenantUi.getBranch()} <br>Commit: ${tenantUi.getHash()}")
    ]
...
private void _postDeployConfigure(TenantUi tenantUi) {
  stage('[UI] Update keycloak redirects') {
    ...
    tenantsToRedirect.each { redirectTenantId ->
      _updateKeycloakClientConfiguration(tenantUi)
    }
  }
}
...
private void _updateKeycloakClientConfiguration(TenantUi tenantUi, String currentTenantId) {
  ...
  Map realm = client.get(realmUrl, headers).body
  ...
  if (realm) {
    ...
    String updateRealmUrl = keycloak.generateUrl("/admin/realms/${currentTenantId}/clients/${realm['id']?.first()}")
...
private String _buildTenantOptionsJson(OkapiTenant tenant, boolean isConsortia, boolean singleConsortiaUI = false) {
