                                        ...
                                        withCredentials([[$class           : 'AmazonWebServicesCredentialsBinding',
                                            ...
                                            if (params.testrailRunID && params.testrailProjectID) {
                                                ...
                                                withCredentials([usernamePassword(credentialsId: 'testrail-ut56', passwordVariable: 'TESTRAIL_PASSWORD', usernameVariable: 'TESTRAIL_USERNAME')]) {
                                                    sh "\$HOME/.yarn/bin/cy2 run --config projectId=stripes --key secretCypressKey --parallel --record --ci-build-id ${customBuildName.replace(' ', '_')} --headless --browser ${browserName} ${params.cypressParameters}"
                                                }
                                            } else {
                                                sh "\$HOME/.yarn/bin/cy2 run --config projectId=stripes --key secretCypressKey --parallel --record --ci-build-id ${customBuildName.replace(' ', '_')} --headless --browser ${browserName} ${params.cypressParameters}"
                                            }
