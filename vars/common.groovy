...
boolean waitKubernetesResourceStableState(String resource_type, String resource_name, String namespace, String replica_count, String max_wait_time){
    sh(script: "start_time=\$(date +%s); while [[ \$((kubectl get ${resource_type} ${resource_name} -n ${namespace} -o=jsonpath='{.status.availableReplicas}') | grep -q '^${replica_count}\$' && echo ${replica_count}) -ne ${replica_count} ]]; do current_time=\$(date +%s); if [[ \$((current_time - start_time)) -gt ${max_wait_time} ]]; then echo \"Deployment did not become stable within ${max_wait_time} seconds.\"; exit 1; fi; sleep 20s; done\n")
}
